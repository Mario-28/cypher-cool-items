...
export function buildEquipmentPanel(item, isGM) {
  const data = CoolItemData.get(item);
  const sys = item.system || {};

  const rarity = data.rarity || 'common';
  const level = data.level ?? '';
  const equipmentType = data.equipmentType || 'tools';
  ...
  topRow.innerHTML = `
    ...
    <div class="cci-info-block">
      ${isGM
        ? `<input type="text" name="name" value="${item.name}" class="cci-name-input" placeholder="Equipment Name">`
      ...
      <div class="cci-info-fields">
        <div class="cci-field-pair">
          <label>Type</label>
          ${isGM
            ? `<select data-prop="equipmentType">${opts(CCI_CONFIG.equipmentType, equipmentType)}</select>`
            : `<div class="cci-readonly cci-equipment-type-badge">${CCI_CONFIG.equipmentType[equipmentType]}</div>`
          }
        </div>
        <div class="cci-field-pair">
          <label>Rarity</label>
...
export function bindEquipmentEvents(panel, item, isGM) {
  // Equipment-specific events (if needed in the future)
